-- Comandos a adicionar:
    -- Mexer no comando para fazer com que a senha seja adicionada sem precisar coloca-la
    -- Restrições nas colunas da tabela, para ter dados condizentes com a realidade

        -- LOJAS:
            -- Adicionar latitudes/longitudes negativas, restringir graus
            -- Adicionar restrição para "logo_ultima_atualizacao" em que a data minima e 1/1/2020.
            -- Adicionar uma restrição específica na coluna “endereco_fisico” na tabela “lojas”: você deve criar uma restrição que garanta que pelo menos uma das colunas de endereço (web ou físico) estejam preenchidas, ou seja: apesar dessas colunas não serem obrigatórias, pelo menos uma deles deve estar preenchida. 

        -- PRODUTOS:
            -- Adicionar restrição a "preco_unitario" para que não aceite valores negativos.
            -- Adicionar restrição a "imagem_ultima_atualizacao" em que a data minima é 1/1/2020.

        -- PEDIDOS:
            -- Adicionar restrição para que "data_hora" seja dia, hora, minuto
            -- Adicionar restrição para que "status" seja CANCELADO, COMPLETO, ABERTO, PAGO, REEMBOLSADO e ENVIADO 
        
        -- CLIENTES:
            -- Adicionar restrição para que "email" possua um caracter especial "@"
            -- Adicionar restrição para que "telefone" não permita "-" e "()"

        -- ENVIOS:
            -- Adicionar restrição para que "status" seja CRIADO, ENVIADO, TRANSITO e ENTREGUE.
...
-- Comando para criar a restrição 
...
-- Comando para criar uma restrição para que a coluna produto_id não aceite valores negativos.

ALTER TABLE lojas.pedidos_itens
ADD CONSTRAINT preco_unitario_not_negative CHECK (preco_unitario >= 0),
ADD CONSTRAINT not_negative_quantidade_estoque CHECK (quantidade_estoque >= 0);
